...
router.post('/adminlogin',(req,res)=>{
	...
	var sql="select * from admins where "
	+"adminname=? and adminpwd=?";
...
router.post('/registe',(req,res)=>{
	...
	if(result.length<1){
		...
		{	
			var sql="INSERT INTO usermsg(username,usernc,userjj,userimg,userjf) VALUES (?,'还未设置昵称','去添加个人简介','http://127.0.0.1:3000/img/userimg/def.png',10) ";
			pool.query(sql,[$uname],(err,result)=>{
			if (err) throw err;
			if (result.affectedRows>0)
			{
...
router.post('/mymsg',(req,res)=>{
	...
	var sql="select * from usermsg where "
	+"username=?";
